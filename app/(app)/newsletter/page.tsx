...
import { sanityClient, newsletterPostsQuery, featuredNewsletterPostsQuery, mainFeaturedNewsletterPostQuery, searchNewsletterPostsQuery, type Post } from '@/lib/sanity'
...
export default function NewsletterPage() {
    ...
    const fetchInitialData = async () => {
        try {
            ...
            // Buscar post principal em destaque da newsletter
            const mainFeatured = await sanityClient.fetch(mainFeaturedNewsletterPostQuery)

            // Buscar posts em destaque para sidebar (excluindo o principal)
            const featured = await sanityClient.fetch(featuredNewsletterPostsQuery)
            const sidebarFeatured = featured.filter((post: Post) => post._id !== mainFeatured?._id)

            // Buscar posts recentes de newsletter (excluindo os em destaque)
    ...
    const handleSearch = async (term: string) => {
        ...
        try {
            ...
            const searchResults = await sanityClient.fetch(searchNewsletterPostsQuery, {
                searchTerm: `*${term}*`
            })
            ...
            console.error('Error searching newsletter posts:', error)
        } finally {
            setLoading(false)
        }
    }
    ...
    const loadMorePosts = async () => {
        ...
        try {
          ...
          _id, title, slug, description, mainImage, thumbnail, generatedImageUrl, publishedAt, author, featured, category
            ...
            console.error('Error loading more newsletter posts:', error)
        } finally {
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="text-center mb-12">
                        ...
                        <p className="text-xl text-gray-300 mb-8">
                            Fique por dentro das novidades exclusivas da nossa newsletter
                        </p>
                    </div>
                            ...
                            <div className="mb-12">
                                <div className="flex items-center justify-between mb-8">
                                    <h2 className="text-3xl font-bold text-white">
                                        {searchTerm ? `Resultados para "${searchTerm}"` : 'Últimas Edições'}
                                    </h2>
                                ...
                                {displayedPosts.length === 0 ? (
                                    <div className="text-center py-20">
                                        ...
                                        <p className="text-gray-400 text-xl">
                                            {searchTerm ? 'Nenhuma edição encontrada.' : 'Nenhuma edição disponível.'}
                                        </p>
                                    </div>
                                ) : (
